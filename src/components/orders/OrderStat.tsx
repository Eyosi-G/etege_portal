...
import { useFetchOrderStatQuery } from '../../service/api/orderService'
import OrderSummerizeCard from './OrderSummerizeCard'

const OrderStat = () => {
    const { data: userData } = useAppSelector(state => state.user)
    const { data: orderStat } = useFetchOrderStatQuery(userData ? userData.token : skipToken)
    return (
        <div>
            <div className='grid grid-cols-2 md:grid-cols-4 py-4 gap-2'>
                <OrderSummerizeCard
                    amount={(orderStat?.pending || 0)}
                    text="Pending"
                    color='bg-blue-200'
                    icon={
